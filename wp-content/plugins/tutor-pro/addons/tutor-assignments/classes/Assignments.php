...
class Assignments extends Tutor_Base {
	...
	public static function get_assignment_result( $assignment_id, $user_id ) {
		...
		$submit_id            = $submitted_assignment[0]->comment_ID ?? 0;
		$is_reviewed          = get_comment_meta( $submit_id, 'evaluate_time', true );
		if ( ! $is_reviewed ) {
			return 'pending';
		}

		$pass_mark  = intval( tutor_utils()->get_assignment_option( $submitted_assignment->comment_post_ID ?? 0, 'pass_mark' ) );
		$marks      = self::get_assignment_earned_marks( $assignment_id, $user_id );
		$given_mark = intval( $marks->earned_marks ?? 0 );
